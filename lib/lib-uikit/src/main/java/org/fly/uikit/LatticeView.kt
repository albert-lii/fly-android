...
open class LatticeView : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var w = MeasureSpec.getSize(widthMeasureSpec).toFloat()
        var h = MeasureSpec.getSize(heightMeasureSpec).toFloat()
        if (oritenation == 0) {
            w = paddingLeft.toFloat() + paddingRight.toFloat()
            ...
            h = paddingTop.toFloat() + paddingBottom.toFloat()
            for (i in 0 until totalCount) {
                h += cellWidth
                if (i < (totalCount - 1)) {
                    h += cellSpace
        ...
        setMeasuredDimension(Math.ceil(w.toDouble()).toInt(), Math.ceil(h.toDouble()).toInt())
    }
