...
public class SlidingLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider, IBanner {
     ...
     * helps {@link androidx.recyclerview.widget.LinearLayoutManager} make those decisions.
    ...
    protected void setItemViewProperty(View itemView, float targetOffset) {
        ...
        // add by Andy She,fix https://console.firebase.google.com/u/0/project/binance-f2287/crashlytics/app/
        // android:com.binance.dev/issues/9e47e1c9c9254c95ce444b2ff426a04f?time=last-seven-days&versions=1.42.0%20(14200)
        try {
     ...
     */
    public SlidingLayoutManager(int orientation, boolean reverseLayout) {
    ...
    public SlidingLayoutManager(float minScale, int itemSpace, int orientation) {
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, RecyclerView.State state, int position) {
        if (mOrientation == VERTICAL) {
            // 解决首页公共滚动问题，只适配了向上滚动
            recyclerView.smoothScrollBy(0, getOffsetToPosition(position), null);
        }else {
    ...
    private final boolean mShouldReverseLayout = false;
    public int getOffsetToPosition(int position) {
        if (mInfinite) {
            final int realPos ;
            if (!mShouldReverseLayout){
                if (position==0 && getCurrentPosition()>position){
                    realPos = getItemCount() - getCurrentPosition();
                }else {
                    realPos = position - getCurrentPosition();
                }
            }else {
                realPos = getCurrentPosition() - position;
            }
            return (int) (((getCurrentPositionOffset() + realPos) *
